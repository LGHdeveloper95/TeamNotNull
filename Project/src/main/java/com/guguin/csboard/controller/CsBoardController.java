...
public class CsBoardController {
 ...
 // 게시판 목록 (페이징 처리)
    ...
    /*
    @GetMapping("/myPosts")
    public String myPosts(@RequestParam(value = "page", defaultValue = "1") int page,HttpSession session, Model model) {
        // 세션에서 로그인된 사용자 ID와 기업 ID 가져오기
        String loggedInUserId = (String) session.getAttribute("userid");
        String loggedInComId = (String) session.getAttribute("comid");
        int totalRecords = csBoardMapper.getTotalBoardCount();
